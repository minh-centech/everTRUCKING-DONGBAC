...
namespace cenCommonUIapps.cenLogistics.Forms
{
    public partial class frmReportViewer : Form
    {
        ...
        private void ExportTemplate()
        {
            ...
            int soDongCanThem = ugBaoCao.Rows.VisibleRowCount - 1; // 1 dòng mẫu đã có
            ...
            // Copy định dạng
            for (i = 1; i <= ugBaoCao.Rows.VisibleRowCount - 1; i++)
            {
            ...
            // Điền dữ liệu theo số dòng trên grid
            int iVisible = 0;
            int iRow = 0;
            while (iVisible <= ugBaoCao.Rows.Count - 1)
            {
                if (!ugBaoCao.Rows[iVisible].IsFilteredOut)
                {
                    ro = sh.GetRow(SoDongBatDau + iRow);
                    foreach (DataRow dc in dtCauTrucCot.Rows)
                    {
                        if (!cenCommon.cenCommon.IsNull(dc["TenCotExcel"]) && !String.IsNullOrWhiteSpace(dc["TenCotExcel"].ToString()))
                        {
                            ...
                            if (dc["Ma"].ToString().ToUpper() == "STT")
                            {
                                ro.GetCell(SoCotExcel).SetCellValue(iRow + 1);
                            }
                            else
                            {
                                switch (dc["KieuDuLieu"].ToString())
                                {
                                    case "3": //Số nguyên
                                        if (!cenCommon.cenCommon.IsNull(ugBaoCao.Rows[iVisible].Cells[dc["Ma"].ToString()].Value))
                                        {
                                            long.TryParse(ugBaoCao.Rows[iVisible].Cells[dc["Ma"].ToString()].Value.ToString(), out long lGiaTri);
                                            ro.GetCell(SoCotExcel).SetCellValue(lGiaTri);
                                        }
                                        break;
                                    case "4": //Số thực
                                        if (!cenCommon.cenCommon.IsNull(ugBaoCao.Rows[iVisible].Cells[dc["Ma"].ToString()].Value))
                                        {
                                            double.TryParse(ugBaoCao.Rows[iVisible].Cells[dc["Ma"].ToString()].Value.ToString(), out double dGiaTri);
                                            ro.GetCell(SoCotExcel).SetCellValue(dGiaTri);
                                        }
                                        break;
                                    case "2": //Ngày tháng
                                        if (!cenCommon.cenCommon.IsNull(ugBaoCao.Rows[iVisible].Cells[dc["Ma"].ToString()].Value))
                                        {
                                            DateTime.TryParse(ugBaoCao.Rows[iVisible].Cells[dc["Ma"].ToString()].Value.ToString(), out DateTime dGiaTri);
                                            ro.GetCell(SoCotExcel).SetCellValue(dGiaTri);
                                        }
                                        break;
                                    default:
                                        ro.GetCell(SoCotExcel).SetCellValue(ugBaoCao.Rows[iVisible].Cells[dc["Ma"].ToString()].Value.ToString());
                                        break;
                                }
                            }
                            if (dtData.Columns.Contains("rBold") && !cenCommon.cenCommon.IsNull(ugBaoCao.Rows[iVisible].Cells["rBold"].Value))
                            {
                                IFont font = wbMau.CreateFont();
                                font.FontHeightInPoints = 10;
                                font.FontName = "Times New Roman";
