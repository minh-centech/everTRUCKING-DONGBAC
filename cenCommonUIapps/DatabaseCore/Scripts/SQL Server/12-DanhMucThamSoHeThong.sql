...
begin
	...
	select ID, IDDanhMucDonVi, Ma, Ten, GiaTri, GhiChu, CreateDate, EditDate from DanhMucThamSoHeThong 
		where IDDanhMucDonVi = @IDDanhMucDonVi and case when @ID is not null then ID else 0 end = ISNULL(@ID, 0) 
	order by Ma;
end
go
...
begin
	set nocount on;
	select @GiaTri = GiaTri from DanhMucThamSoHeThong where IDDanhMucDonVi = @IDDanhMucDonVi and Ma = @Ma;
end
go